...
-   Logistic regression won't find and model interaction effects unless they are explicitly mapped as features (we did this for some features).

-   Plain linear regression models can suffer from multicollinearity and high dimensionality in the feature set, but **regularization** is almost always applied to model coefficients to avoid overfitting.

    -   **L1 (lasso)** regularization can shrink model coefficients to zero, effectively dropping predictors from the model and performing feature selection.
...
Logistic regression is normally fit on all training observations at once: The model coefficients are all estimated in one go using a solver algorithm (implemented with [LogisticRegression](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html) in scikit-learn). This can be a bit slow with a large dataset like ours, especially since we'll fit hundreds of models for hyperparameter tuning. To save time, we'll train our logistic regression (and SVM) models with **stochastic gradient descent:**
...
-   The model will be trained with the `.partial_fit()` method, which trains the SGDClassifier on one training observation only. We'll call the method repeatedly to train the model for several epochs. Our function will handle early stopping manually by recording each epoch's score. By default, 10 epochs with less than 0.0001 improvement in the validation error will terminate the model training.
    ...
    -   The class weight for each class will be inversely proportional to its class frequency, calculated automatically by a scikit-learn utility (only using the training data to avoid leakage). Using these class weights, we will create a sample weights vector for every observation, and pass this to the model training and scoring functions.

-   If the function is ran for an Optuna trial, it will return the average CV score of the parameter set (best epochs, not the last ones). It will also perform pruning if directed by the Optuna pruner:
...
-   The `loss` parameter sets the training loss function. For probabilistic logistic regression, we will use log loss, which compares the probability predictions to the actual target labels. We will also use log loss to score our predictions on the validation set.
...
-   We'll tune `alpha` between a very small float value of 5e-5, and 0.5, though the optimal value is likely close to 0. Therefore, we will instruct Optuna to sample from the log domain:
    ...
    -   The logarithm of the search space will be taken, values will be sampled from this space, and the exponent will be taken to return the original value. This will make lower values more likely to be suggested. We'll do this for other hyperparameters that usually take an optimal value close to 0.
...
-   We set a pruner algorithm: The pruner decides whether to complete a trial, or prune it based on its intermediate validation scores. This saves us a great deal of time by stopping unpromising trials early.
    ...
    -   We use the **Hyperband** pruner algorithm, which is suggested for the TPE sampler. In short, hyperband allocates a finite budget equally to each hyperparameter configuration. There is the option to increase the budget, at the expense of eliminating configurations more aggressively (see the [documentation](https://optuna.readthedocs.io/en/stable/reference/generated/optuna.pruners.HyperbandPruner.html) and [original paper](https://www.jmlr.org/papers/volume18/16-558/16-558.pdf) for more detail).
...
We can finally run the hyperparameter optimization. It is recommended to run the TPE algorithm with 100 to 1000 trials. The models in the final edition of this report were all tuned with 500 trials.

-   With pruning in place, even if improvement stops after a few trials, the remaining trials will mostly be pruned.

-   For logistic regression, one study of 500 trials usually took around 10-15 minutes. The SGDClassifier was fully parallelized with `n_jobs = -1`.
...
The trials were exported to .csv for future use, and the best set of hyperparameters will be reloaded.
...
The median best epoch for the 3 CV folds was 26, which is the number of epochs we'll use in our final logistic regression model, defined below.

-   We combine our preprocessing pipeline with our logistic regression model, to perform all steps in one go.

-   We'll train our final models with the optimal hyperparameters, and the optimal number of epochs we derived from crossvalidation, with no early stopping.

    -   Even with the built-in validation disabled, SGDClassifier performs early stopping based on the training loss.

    -   This can be misleading, as lower training loss doesn't necessarily translate into better predictive performance on unseen data (in the case of overfitting). To disable this feature, we set an arbitrarily high number for the `n_iter_no_change` parameter.
...
-   SVM does not natively output probability predictions between 0 and 1. Instead, it calculates a **decision function,** based on the distance of an observation from the decision boundaries. This has implications for our performance evaluations, which we will discuss in model testing.
...
-   After the kernel approximation, the SGDClassifier will act as a linear SVM. For this, we have to change our loss metric to **hinge loss.**
...
The validation function and the Optuna tuning process is pretty much the same with logistic regression. The code is available below.
...
The optimal hyperparameter set has lower regularization strength compared to logistic regression, but a higher ratio of L1 regularization. This could suggest the RBF transformation made our feature set more redundant, but it's likely not directly comparable across different algorithms.

We create the final SVM model, combining it with the preprocessing & kernel approximation pipeline.

-   We also wrap our SVM model in a [CalibratedClassiferCV](https://scikit-learn.org/stable/modules/generated/sklearn.calibration.CalibratedClassifierCV.html) object. This will approximate probability predictions from the SVM decision function, so we can calculate some performance metrics and compare with other models. The actual probabilities acquired in this manner may be unreliable.
