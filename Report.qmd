...
-   Tree-based models learn relationships & make predictions by splitting the data according to the predictor values.
    ...
    -   This means they can model non-linear relationships, and find interaction effects even if they are not explicitly mapped as features.

    -   However, they can't extrapolate relationships between predictors and the target outside of the observed training data.

        -   For example, for a feature X and target Y, a relationship of Y = 2X will be easily modeled and predicted by a linear model, even if X takes a very large value not seen in training. A tree-based model, however, will continue predicting according to the highest X value seen in training.

-   No hard assumptions are made about the data, though curse of dimensionality can make it harder to identify the optimal splits and potentially hamper prediction performance.
...
-   We can simply pass the validation set while training an XGBoost model, which automatically handles early stopping, records the best validation score and the best round.
...
-   XGBoost supports GPU training, which will save us some training time. With 1000 trials, the Optuna study took roughly 25-30 minutes.
...
-   `learning_rate` is is the magnitude of the model update after each boosting round, similar to in SGD training. The same considerations as explained before apply. A general suggestion is to tune the XGBoost LR between 0.05 and 0.3 for most problems. It's also possible to use LR scheduling, but we will save this for the NN model.
...
-   `gamma` is a special regularization parameter for trees. It is the minimum loss reduction required to justify another split. The optimal value is often closer to zero.

-   `alpha` and `lambda` are L1 and L2 regularization respectively, applied to model weights. L1 is 0 by default, and L2 is 1 by default. We'll tune both over a wide interval to try different blends of L1 and L2 regularization.
...
-   `subsample` and `colsample_bytree` are the subsampling parameters, both set to 1 by default (meaning no subsampling).
    ...
    -   A lower value for `colsample_bytree` will subsample the training features by that ratio in every boosting round.
...
Our final model is a neural network. There is much to explain about how NNs work, and there are many complex architectures, but I'll summarize the main idea starting from a simple linear regression:
...
-   One such linear regression equation is equivalent to **one neuron / linear unit** in a NN. A **layer** is an organization of numerous neurons / units, or other transformations.

-   The inputs of a NN are equivalent to the original features, as they are fed into the network. The **output layer** is the last layer that gives the final output / prediction of the network.

-   A **hidden layer** is any layer between the network inputs and the output layer. The inputs of neurons in a hidden layer consist of the outputs of the previous layer (for the first hidden layer, simply the input features). Hidden layer neurons apply their weights and bias terms to the inputs, and pass their outputs to the next layer. A NN with at least one hidden layer is a **deep NN.**

    -   We can think of a multiple linear regression model as a single neuron / linear unit with multiple inputs and one output.

    -   When we train a simple linear regression model, we estimate a single coefficient for each feature, and one intercept term. In a NN, we estimate a weight for each input of each neuron, and a bias term for each neuron.

**Activation functions** can be applied after a layer, to transform its outputs before passing them on to the next layer as inputs. Many activation function options exist, which apply different mathematical transformations.
...
Of course, this is the simplest summary of a typical NN. [Here](https://d2l.ai/index.html) is an excellent source for more information on many aspects of deep NNs.
...
-   Each hidden layer will be followed by a **SELU** activation function layer. This stands for "scaled exponential linear unit". The SELU activation will enable us to model non-linear relationships, while also enforcing a self-normalizing property on the layer outputs (see [original paper](https://arxiv.org/abs/1706.02515)).
    ...
    -   Neural networks can be prone to numerical instability issues, so it's a common approach to apply forms of scaling / normalization **inside the network**, to the outputs of layers, before passing them on to the next layer (the scaling / normalization we apply to our features in preprocessing can also be applied as the first layer of a network).

    -   The most common method for in-network normalization is adding **batch normalization layers**, but apparently it can conflict with the application of **dropout layers**.
...
-   The SELU activation after each hidden layer will be followed by a **dropout layer**.
    ...
    -   The probability of dropping each layer input is another hyperparameter we will tune, as `dropout`. Obviously, a higher probability will result in more severe regularization and possible underfitting.
